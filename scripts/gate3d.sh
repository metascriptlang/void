...
# A tap through the renderer that drew the frame: a snapping, panning camera with a margin, so
# the blit carries a remainder, and the logs turning. Each tap is the framebuffer pixel centre
# under a target mesh; the entry prints one PICK row per tap and whether it hit what it should.
write_pick_entry() {
	{
		echo "// Generated by scripts/gate3d.sh. Edit the script, not this file."
		echo "import {"
		...
		echo "} from \"../campfireScene\";"
		echo "import { PixelArtSettings } from \"../../../src/void3d/pixelArtRenderer\";"
		echo "import { NodeId } from \"../../../src/void3d/scene\";"
		...
		echo "	const at: Vec3 = campfirePixelOf(on);"
		...
		echo "	const right: boolean = expectHit && hit.node.index == owner.index &&"
		echo "		hit.node.generation == owner.generation;"
		echo "	const verdict: string = right ? \"PASS\" : \"FAIL\";"
		echo "	console.log(\`PICK \${label} \${verdict} pixel=(\${x}, \${y}) node=\${hit.node.index} mesh=\${hit.mesh.index} distance=\${hit.distance}\`);"
		echo "}"
		echo ""
		echo "function frame(): void {"
		echo "	frameCampfire();"
...
run_pick() {
	if [ ! -f "$CAPTURE/campfirePick.ms" ]; then
		skip "pick: $CAPTURE/campfirePick.ms is missing"
		return
	fi
	...
	pass "pick: 3 taps on the drawn campfire, snapping and panning, picked what they should"
