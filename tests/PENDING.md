...
| style:line-length | T0 | CODE-STYLE asks for <=100 columns. Re-measured 2026-09-25 with the P0 command under a UTF-8 locale, `LC_ALL=C.UTF-8 awk 'length > 100' $(git ls-files '*.ms') | wc -l` (the C locale counts an em dash as three and reads 248): **246** lines across **40** MetaScript files; 53 are the one-row-per-scene data table. A compliant cut would touch 31 files, mostly formatting pre-existing imports, signatures and data rows, so P2 rejected that unrelated churn during renderer review. P6 "Modules and hardening" owns the repository-wide mechanical pass | P6 | 2026-09-23 |
| style:span-of-interface-param | T0 | `sortByZ` in `src/void2d/render.ms` keeps a `Node2D[]` parameter where CODE-STYLE §14 asks for `Span<Node2D>` (P3.5 follow-up F-c): passing an interface array as a span emits an uncast `void **`, which emsdk's clang refuses, so the web build failed. Compiler card `../../.inbox/compiler/2026-09-26-interface-array-to-span-void-pointer.md` | compiler | 2026-09-26 |
