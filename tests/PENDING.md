...
| id | tier | reason | phase | date |
|---|---|---|---|---|
| font-collection-model | T0 | P3 step 5 landed the fallback chain, the `Font` value with CSS weight matching, synthetic bold and italic, and opt-in fallback size harmonisation (`src/test/fontCheck.ms`, `syntheticFaceCheck.ms`, golden `text/fontStyles`). Not landed: colour emoji families and explicit-versus-fallback presentation, which wait on step 8's rasterizer decision because stb_truetype reads no colour glyph table; a deferred face answering coverage without being loaded (void loads it on the first miss); whole-grapheme selection, which needs the UCD segmentation P4 wires | P3 | 2026-09-24 |
