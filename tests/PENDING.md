...
| id | tier | reason | phase | date |
|---|---|---|---|---|
| atlas-resize-transient-frame | T2 | a glyph first needed in a second render-target bracket is one frame late because sokol permits only one atlas upload per frame. Labels laid out before a growth also hold UVs normalised to the old atlas for that frame; each corrects on its next sync, and the triggering label is not permanently wrong because `lastAtlasGen` is read before layout. The runner draws three frames, so a golden cannot see either transient | P3 | 2026-09-20 |
