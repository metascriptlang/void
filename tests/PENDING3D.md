...
| `material-saturation-only` | A material changes colour only by a saturation amount, and only on the lit program: the toon block's `y`, applied after lighting. Heaps attaches `h3d.shader.ColorMatrix`, a whole colour matrix, to any pass. The saturation is the one Heaps computes (`tests/oracle/color3d.cases` agrees with `Matrix.colorSaturate`); what is missing is the rest of the matrix and every other program. | `src/void3d/shader3d.glsl` | when a caller needs another colour change on one object, or on a billboard or particle |
