...
- **Colour emoji**, a compile-time module, decided at P3's review. stb_truetype reads no colour table, so the module reads them itself: CBDT/CBLC and sbix bitmap strikes decoded by `stb_image`, already a void dependency (`src/assets/image.c`), and COLRv0 as layers of stb outlines, each tinted by its palette entry. Tiles go into the RGBA pages P3 reserved (P3 "Atlas page kinds") and draw at a whole-pixel origin with no gamma correction, as GPUI does (GPUI.md:51). Explicit-versus-fallback presentation comes with it (GHOSTTY.md:24), VS15/VS16 over P4's grapheme segmentation, and so do deferred faces, so an emoji family answers coverage before it loads.
