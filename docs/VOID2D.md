...
The editor scene, `tests/bench/benchEditor.ms` (`fd0dafa`, gated in `tests/bench/baseline.json`):
...
`TextLayout` copy per selection row per frame that the design pass found (`39eaafa`); `present` is
taken again with the A/B below.
