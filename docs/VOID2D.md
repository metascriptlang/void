...
- ~~**Per-frame vertex cap**~~ — **closed in P1** (`747127f`). The fixed 65 536-vertex dynamic buffer grows by `max(2×)` instead, with a 192 MB cap and a dropped frame plus an error on stderr past it. The golden `regress/vertexCap` moved: the grid fills the canvas. Re-measured in a **debug** build, where sokol's validation layer is linked, both `regress/vertexCap` and `regress/nodeCap` now capture normally — the `VALIDATION_FAILED` panic at `sokol_gfx.h:23960` is gone, so `tests/PENDING.md debug-abort:vertex-cap` was deleted.
- ~~**GPU calls are issued while the tree is walked**; a text change destroys and recreates its GPU buffer~~ — **closed in P1** (`747127f`). The walk's only output is now the command, effect and vertex streams; `void2dReplay` is the single draw-issuing function and runs after the walk has ended. No node owns a GPU buffer, so a text change rewrites a `float32[]` on the node and nothing else. The behaviour is fixed; the **T1 assertion that holds it fixed is not written yet** — `tests/PENDING.md` keeps `walk-issues-gpu-calls`, `text-buffer-churn` and `upload-per-bracket` open against the T1 tier, which lands later in this same phase.
...
| GPU buffers alive | constant in node count | **3**, was 126 | met |
