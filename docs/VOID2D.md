...
2. **Guardrail 9 remains one measured surface of seven.** Both web backends now build and
   WebGL2 draws the demo headlessly, but neither web path has golden readback; headless WebGPU
   has no adapter on this machine. Those are liveness/build results, not cross-backend
   conformance, so the six non-D3D11 surfaces remain explicit SKIPs against the 67-image set.
