...
**Unblocks.** P4; and the Neon host's Yoga measure callback, which is the gate on a Void-backed Neon app rendering text at all. The host can bind it now, but two things under it move in P4: the cost of one measure call, which recomputes the face's decoration metrics and copies the layout, and the font API's shape under CODE-STYLE §14 (REVIEWS.md P3 F2 and F9).
...
**Measure.** First, before any P4 change: `scripts/bench-ab.sh` of the pre-P3 control `8a473f3` against the head on a box checked quiet. Both arms of P3's A/B read about 2.2× P2's absolute UI milliseconds, and P4's `present` becomes P5's baseline, so the shift is explained or re-taken here (REVIEWS.md P3 F10). Then a new bench scene — 200 lines × 80 columns with runs, a selection and a caret: draw calls, instances, `present`. It becomes the scroll case P5 is measured on.
