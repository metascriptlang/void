...
The interesting number was the UAX #14 one, and it decided the rule. The U+0020 rule P3 shipped agreed on 12 566 of 19 346 `LineBreakTest.txt` rows; GPUI's rule, after a space or before any non-word char (GPUI.md:49), on fewer, 11 398, because it wraps ideographs but breaks inside every alphabet its word list does not name. On those numbers the human chose UAX #14 (2026-09-26), which now agrees on every row. That is the use of a pass rate: the number is the argument for or against a rule, instead of nobody knowing.
