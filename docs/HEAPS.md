...
| Text | **BASIC → P3 in progress** | float unrounded layout, GPOS kerning, wrap on the layout, quarter-pixel x-variants and a whole-pixel baseline, R8 pages reclaimed whole and failing loud when full, `textWidth`/`calcTextWidth`/`splitText`/per-glyph x from the painted layout (P3 step 3); GPUI's gamma/contrast correction (P3 step 4), a per-codepoint fallback chain loaded on a miss (P3 step 5); still no `Font` value with weight/style/features, no runs | `FontChar` + kerning list, `needsRebuild`, `HtmlText`, SDF fonts; but baked atlases, Int metrics, one page | High — replaced at [VOID2D.md](VOID2D.md) P3 |
