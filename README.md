...
> **Status: early, internal.** Void is built alongside its own use cases, and the API still moves without notice. The Android shell and `void3d` are the newest and roughest parts. Read it, don't depend on it.
...
`void2d` is one such layer, shipped here: a quad batcher with a `Node2D` tree and text, drawing identical pixels on every backend. `void3d` (camera, mesh, fixed pipelines) is a spike, to be rewritten.
...
src/void3d/     3D layer — camera, mesh, fixed pipelines
src/examples/   thin entries: voidRun(w, h, init, frame)
android/        Kotlin shell — sample app and live wallpaper
```
...
scripts/build-android.sh                # arm64 .so for android/; ENTRY= picks the demo
```
